...
Building a `release` version of httpsrv binary strips out the assert() calls, so in case of bugs or hardware failures or resources (e.g. memory) exhausted that might generate an `HTTP 500 Internal Server Error`.
