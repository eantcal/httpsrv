...
A *functional tests* have been implemented as [BASH script](test/functional_test.sh) in order to verify the main use scenarios. 
It relies on a number of welknown 3pp commands/tools including `grep`, `awk`, `sed`, `unzip`, `curl`, `jsonlint`, `sha256sum`. 
To execute the functional tests httpsrv binary must be running (by default bound on localhost:8080).
...
To install successfully HttpSrv the following software component is required on the installation computer:
...
`CMakeLists.txt` and [Visual Studio 2019 C++ project](https://github.com/MicrosoftDocs/cpp-docs/blob/master/docs/build/creating-and-managing-visual-cpp-projects.md) project file (`httpsrv.vcxproj`) and solution solution (`httpsrv.sln`) have been provided.
...
Lines of code excluding 3pp and standard libs:
