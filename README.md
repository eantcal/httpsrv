...
When `accept()` accepts a connection, `run()` gets a new `TcpSocket` handle (a smart pointer to the actual obejct), which represents the TCP session, and creates a new HTTP session handled by a dedicated instance of class `HttpSession`.
